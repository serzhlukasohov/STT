...
test('mergeTranscriptionParts rejects failed part placeholders', () => {
  assert.throws(
...
test('mergeTranscriptionParts rejects empty transcription parts', () => {
  assert.throws(
    () => mergeTranscriptionParts([
      { text: 'First part.' },
      { text: '   ' }
    ]),
    /part 2 returned empty text/i
  );
});
